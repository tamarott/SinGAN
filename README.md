...
SinGAN can also be used for a line of image manipulation tasks, for example:
 ![](imgs/manipulation.PNG)
This is done by injecting an image into the already trained model. See section 4 in our [paper](https://arxiv.org/pdf/1905.01164.pdf) for more details.
...
###  Train
To train the SinGAN model on your own image, put the desired training image under Input/Images, and run
...
###  Random samples
To generate random samples from any starting generation scale, please first train the SinGAN model for the desired image (as described above), then run 
...
To generate random samples of arbitrary sizes, please first train the SinGAN model for the desired image (as described above), then run 
...
To harmonize a pasted object into an image (See example in Fig. 13 in [our paper](https://arxiv.org/pdf/1905.01164.pdf)), please first train the SinGAN model for the desire background image (as described above), then save the naively pasted reference image and it's binary mask under "Input/Harmonization" (see saved images for an example). Run the command
...
To edit an image, (See example in Fig. 12 in [our paper](https://arxiv.org/pdf/1905.01164.pdf)), please first train the SinGAN model on the desire non-edited image (as described above), then save the naive edit as a reference image under "Input/Editing" with a corresponding binary map (see saved images for an example). Run the command
...
To transfer a paint into a realistic image (See example in Fig. 11 in [our paper](https://arxiv.org/pdf/1905.01164.pdf)), please first train the SinGAN model on the desired image (as described above), then save your paint under "Input/Paint", and run the command
...
```
This will automatically train a SinGAN model to correspond to the 4x upsampling factor (if not exist already).
