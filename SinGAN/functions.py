...
def save_image(real_cpu, receptive_field, ncs, epoch_num, file_name):
    ...
    rect = patches.Rectangle((0, 0), receptive_field, receptive_field, linewidth=5, edgecolor='r', facecolor='none')
    ax.add_patch(rect)
    ax.axis('off')
...
def generate_dir2save(opt):
    ...
    if (opt.mode == 'train') | (opt.mode == 'SR_train'):
        ...
        dir2save = 'TrainedModels/%s/scale_factor=%f_paint/start_scale=%d' % (opt.input_name[:-4],
                                                                              opt.scale_factor_init,
        ...
        dir2save = '%s/RandomSamples_ArbitrarySizes/%s/scale_v=%f_scale_h=%f' % (opt.out, opt.input_name[:-4],
                                                                                 opt.scale_v, opt.scale_h)
